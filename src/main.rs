...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let repos = github::get_user_repos(&client, &token, &github_user).await?;
    let mut repos_to_add = Vec::new();
    
    for repo in repos {
        if let Some(name) = repo["name"].as_str() {
            if github::get_repo_branch(&client, &token, &github_user, name, &repo_branch).await? {
                let repo_info = RepositoryInfo {
                    name: name.to_string(),
                    languages: github::get_languages(&client, &token, &github_user, name).await?,
                    ...
                    commits: github::get_commits(&client, &token, &github_user, name).await?,
